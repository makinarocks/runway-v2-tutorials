...
사용법:
    ...
    # 명시적 엔드포인트 지정
    python test_inference.py --endpoint https://inference.<domain>/api/<proj>/<ep>/<deploy>
"""
...
def main():
    ...
    if not token:
        print("[test_inference] 토큰 없음. --token / env RUNWAY_API_KEY / .env OPENBAO_TOKEN 필요", file=sys.stderr)
        sys.exit(1)
