...
// 엔드포인트 설정은 브라우저에 남긴다. 튜토리얼을 여러 번 오가며 쓰는 화면이라
// 새로고침할 때마다 URL 을 다시 넣게 하지 않는다. 자격 증명은 여기 들어가지 않는다.
const ENDPOINTS_KEY = 'energy-gui.endpoints';
...
function loadSavedEndpoints() {
  try {
    ...
    return v && v.inferenceEndpoint ? v : null;
  } catch {
    return null; // 사생활 보호 모드 등 저장소를 못 읽는 환경
  }
}

function saveEndpoints(v) {
  try {
    window.localStorage.setItem(ENDPOINTS_KEY, JSON.stringify(v));
  } catch {
...
export default function EnergyDemandMLOpsSystem() {
  ...
  const [apiSettings, setApiSettings] = useState(() => loadSavedEndpoints() ?? {
  ...
  // 첫 방문에는 설정 창을 띄우고, 저장해 둔 값이 있으면 띄우지 않는다.
  const [apiSettingsOpen, setApiSettingsOpen] = useState(!loadSavedEndpoints());
