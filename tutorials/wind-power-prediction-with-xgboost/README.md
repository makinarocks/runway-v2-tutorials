...
> ⚠️ **이 내용은 현재 버전 플랫폼에 맞지 않습니다.**
>
> | | |
> |---|---|
> | **대상 플랫폼** | Runway v2.2.0 이전 |
> | **최종 갱신** | 2026-05-07 |
> | **상태** | 현행 플랫폼 미검증 |
>
> 작성 이후 플랫폼이 여러 번 바뀌었고, 특히 **시크릿 주입 방식**과 **의존성 버전 처리**가
> 현행과 다릅니다. 그대로 따라 하면 진행되지 않는 단계가 있을 수 있습니다.
>
> - 이 튜토리얼은 OpenBao API를 **토큰으로 직접 호출**(`hvac`/`urllib`)합니다.
>   현행 튜토리얼은 **OpenBao Agent Injector**가 파일로 마운트하는 방식이고,
>   S3 자격 증명은 프로젝트에 자동 제공되는 `s3-rw` 시크릿에서 받습니다.
> - Python·라이브러리 버전을 고정하지 않습니다. 서빙 런타임(MLServer)과 버전이 어긋나면
>   **오류 없이 예측값만 틀어질 수 있습니다.**
>
> **지금 처음 시작한다면** 루트 [README](../../README.md)의 `energy-demand-prediction-v2`를 사용하세요.
> 이 튜토리얼은 **단일 출력 회귀 예제**로서 유효하여 남겨 두며, 현행화는 v2.5.0 문서 작업에서 다룹니다.
